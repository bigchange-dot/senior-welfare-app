...
### 앱 목적
노인 복지 공고(서울 주요 구청·복지로)를 자동 수집하고, AI로 요약한 뒤 FCM 푸시 알림으로 어르신께 실시간 전달하는 서비스.
...
**크롤링 대상 (v1.1 기준):**
- 복지로: 공공 DATA API (`DATA_GO_KR_KEY`) — `NationalWelfareInformationsV001`
- 노원구청 / 도봉구청 / 중랑구청 / 마포구청 / 은평구청: `_scrape_seoul_board()` 공통 헬퍼
- 성동구청: `requests` + `BeautifulSoup4` HTML 파싱
- 강북구청: `requests` + `BeautifulSoup4` + SAB 봇 감지 우회

**AI 요약 프롬프트 패턴:**
```python
client.models.generate_content(
    model="gemini-3.1-flash-lite-preview",
    contents=prompt,  # 노인 관련 여부 판별 + 30자 이내 속보 요약 or SKIP
)
```
...
│  AppBar: "어르신 알리미"  🔄 │
├─────────────────────────────┤
│  [전체] [복지로] [노원구청] [도봉구청] [중랑구청] ...  ← 필터 칩 바 (가로 스크롤)
├─────────────────────────────┤
│                             │
│  ┌─────────────────────┐    │
│  │ [성동구청]    2시간 전│    │
...
| 탭 | 주요 기능 |
|----|----------|
| **어르신 알리미** | 전체 공고, 필터 칩(전체/복지로/노원/도봉/중랑/마포/은평/성동/강북 — 가로 스크롤), 5건마다 광고 삽입, ♥ 버튼으로 찜 추가 |
| **찜한 공고** | **상단:** 찜한 공고 목록 / **하단:** 설정한 내 지역 공고 목록 — 탭 전환 시 자동 새로고침 |
| **설정** | 지역 선택 9개(복지로·노원·도봉·중랑·마포·은평·성동·강북·전체), FCM 토픽 자동 변경, 알림 ON/OFF |
...
| Flutter 앱 (Android 실기기) | ✅ 확인 완료 | FCM·딥링크 정상 (2026-03-12) |
| Flutter 앱 릴리즈 APK | ✅ 빌드 완료 | 46.7MB (2026-03-12) |
| Flutter 앱 (iOS) | 🔲 미진행 | — |
...
| 항목 | 우선순위 |
|------|----------|
| 신규 구청 5개 스크래핑 URL 실제 확인 및 교체 | 높음 |
| `flutter build appbundle --release` (AAB 빌드) | 높음 |
